// Replace the return block in ProfilePanel.jsx dynamically
const fs = require('fs');
const file = 'C:/Users/User/Desktop/gamification/client/src/components/ProfilePanel.jsx';
let str = fs.readFileSync(file, 'utf8');

const newRender = `    <div className="p-6 rounded-3xl mb-8 shadow-2xl" style={{ background: "var(--panel-bg)", border: "2px solid var(--panel-border)" }}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="flex flex-col items-center justify-center lg:col-span-1 gap-6">
          <div className="w-full rounded-lg border px-4 py-3 shadow-lg flex flex-col items-center justify-center gap-2" style={{ background: "var(--card-bg)", borderColor: "var(--card-border-idle)" }}>
            <span className="text-[10px] uppercase tracking-[0.16em] text-slate-400">Rank</span>
            <div className="rank-badge px-4 py-1 rounded border-2 shadow-lg" style={{ borderColor: tierStyle.borderColor, background: tierStyle.background, boxShadow: tierStyle.glow }}>
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="bg-slate-900/80 border border-yellow-700/80 rounded-lg px-4 py-3 shadow-lg" style={{ borderColor: "var(--panel-border)" }}>
          ...
          <div className="bg-slate-900/80 rounded-lg px-3 py-2 shadow-lg" style={{ borderWidth: 1, borderStyle: "solid", borderColor: "var(--panel-border)" }}>
            <div className="flex justify-between items-center mb-2">
          ...
          <div className="p-4 rounded-lg border-2 shadow-lg" style={{ background: "var(--streak-bg)", borderColor: "var(--streak-border)" }}>
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <button
                ...
                className={\`cinzel font-bold px-5 py-3 rounded-lg border transition-all text-xs flex items-center justify-center gap-1.5 whitespace-nowrap w-full sm:w-auto \${
                  canReroll
...
const startIndex = str.indexOf('<div className="p-6 rounded-3xl mb-8 shadow-2xl"');
const endIndex = str.lastIndexOf('  ProfilePanel.propTypes =');

if (startIndex > -1 && endIndex > -1) {
  // Replace everything between startIndex and endIndex - 5 (to leave line break)
  let updatedStr = str.slice(0, startIndex) + newRender + '\n  );\n}\n\n' + str.slice(endIndex);
