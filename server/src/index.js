...
app.post("/api/city/upgrade-district", async (req, res) => {
  ...
  try {
    ...
    const cost = DISTRICT_UPGRADE_COSTS[currentLevel];
    if ((user.tokens || 0) < cost) {
      return res.status(400).json({ error: "Not enough tokens", code: "insufficient_tokens", required: cost, tokens: user.tokens || 0 });
    }
